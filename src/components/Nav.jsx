      ...
      <button onClick={()=> setMobileMenuActive(!mobileMenuActive)}  className="lg:hidden p-2 focus:ring-2 focus:ring-gray-200 rounded-lg hover:bg-gray-100">
        ...
        {/* MENU LIST */}
          <div className={`${mobileMenuActive? "block":"hidden"} lg:block w-full lg:w-auto`}>
              <ul className="flex flex-col lg:flex-row lg:gap-2 bg-gray-200 lg:bg-transparent text-lg capitalize border border-gray-200 lg:border-none rounded-lg p-4">
                  
                  {ROUTES.map((route, i) => {
                      return (
                          <li className={`rounded py-2 px-3 cursor-pointer ${i === 0 ? "bg-blue-500 text-white lg:bg-transparent lg:text-blue-500" : "hover:bg-gray-100"}`} key={route}>
                              {route}
                          </li>)
                  })}
